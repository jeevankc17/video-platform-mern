...
export const createAPlaylist = createAsyncThunk(
    ...
    async ({ name, description }) => {
        try {
            const response = await axiosInstance.post(`${server}/playlist`, {
                name,
                description,
            });
    ...
    async (playlistId) => {
        try {
            const response = await axiosInstance.patch(
                `${server}/playlist/add/${playlistId}`
            );
            if (response.data?.success) {
                toast.success(response.data.message);
            }
...
export const removeVideoFromPlaylist = createAsyncThunk(
    ...
    async (playlistId) => {
        try {
            const response = await axiosInstance.patch(
                `${server}/playlist/remove/${playlistId}`
            );
            if (response.data?.success) {
                toast.success(response.data.message);
            }
...
export const getPlaylistById = createAsyncThunk(
    ...
    async (playlistId) => {
        try {
            const response = await axiosInstance.get(`${server}/playlist/${playlistId}`);
            return response.data.data;
        } catch (error) {
            toast.error(error?.response?.data?.error);
            throw error;
