...
export const getAllVideos = createAsyncThunk(
    ...
    async ({ userId, sortBy, sortType, query, page, limit }) => {
        try {
            const url = new URL(`${server}/video`);
...
export const publishAvideo = createAsyncThunk("publishAvideo", async (data) => {
    ...
    try {
        const response = await axiosInstance.post("${server}/video", formData);
...
export const deleteAVideo = createAsyncThunk(
    ...
    async (videoId) => {
        try {
            const response = await axiosInstance.delete(`${server}/video/v/${videoId}`);
            toast.success(response?.data?.message);
            return response.data.data;
        } catch (error) {
            toast.error(error?.response?.data?.error);
...
export const getVideoById = createAsyncThunk(
    ...
    async ({ videoId }) => {
        try {
            const response = await axiosInstance.get(`${server}/video/v/${videoId}`);
            return response.data.data;
        } catch (error) {
            toast.error(error?.response?.data?.error);
            throw error;
...
export const togglePublishStatus = createAsyncThunk(
    ...
    async (videoId) => {
        try {
            const response = await axiosInstance.patch(
                `${server}/video/toggle/publish/${videoId}`
            );
            toast.success(response.data.message);
